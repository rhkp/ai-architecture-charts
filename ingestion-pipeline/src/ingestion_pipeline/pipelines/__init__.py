    ...
    with tempfile.NamedTemporaryFile(suffix=".yaml") as tmp:
        ...
        if pipeline_id is None:
            ...
            version_id = [
                ...
                if v.display_name == pipeline_name
            ][0]
        else:
...
def get_pipeline_runs(client: Client, pipeline_name: str):
    ...
    while True:
        ...
        filtered_runs = [
            run
            for run in runs
            if run.pipeline_version_reference
            and run.pipeline_version_reference.pipeline_id == pipeline_id
        ]
    ...
    logger.info(
        f"Collected runs: {[run.run_id for run in pipeline_runs]} for {pipeline_id=}"
    )
...
def delete_pipeline(pipeline_name: str):
    ...
    while True:
        response = client.list_pipeline_versions(
            pipeline_id=pipeline_id, page_token=next_page_token
        )
        ...
        for version in response.pipeline_versions or []:
            ...
            client.delete_pipeline_version(
                pipeline_id=pipeline_id, pipeline_version_id=version.pipeline_version_id
            )
