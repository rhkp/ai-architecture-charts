...
@app.post("/add")
async def add_pipeline(payload: Union[GitHubSource, S3Source, URLsSource] = Body(...)):
    try:
        k8s_name = await asyncio.to_thread(
            k8s.apply_model_as_secret, payload, replace=True
        )
        pipeline_id = await asyncio.to_thread(
            pipelines.add_pipeline, k8s_name, payload.source
        )
...
async def get_pipeline_status(pipeline_name: str):
    try:
        ...
        state = await asyncio.to_thread(
            pipelines.get_latest_run_state, pipeline_name=k8s_name
        )
